...
class SceneDepart extends Phaser.Scene{
	...
	this.load.spritesheet('spritesheet_mario', 'assets/spritesheet_mario.png', { frameWidth: 40, frameHeight: 36 });
	...
	create(){
		...
		this.anims.create({
            key: 'leftmario',
            //frames: this.anims.generateFrameNumbers('mario', { frame: 6, frame: 14 }),
            frames: this.anims.generateFrameNumbers('spritesheet_mario', { start: 6, end: 9 }),
            frameRate: 10,
            repeat: -1
        });
        this.anims.create({
            key: 'turnrightmario',
            frames: [ { key: 'spritesheet_mario', frame: 0 } ],
            frameRate: 20
        });
		this.anims.create({
            key: 'turnleftmario',
            frames: [ { key: 'spritesheet_mario', frame: 9 } ],
            frameRate: 20
        });
        this.anims.create({
            key: 'rightmario',
            frames: this.anims.generateFrameNumbers('spritesheet_mario', { start: 0, end: 3 }),
            frameRate: 10,
            repeat: -1
        });
		...
		if (mario_mort) {
			...
			if (button_p.isDown){ //si on appuie sur a touche P
				...
				if (cursors.left.isDown){
					...
					mario.anims.play('leftmario', true);
				}
				else if (cursors.right.isDown){
					direction='droite';
					mario.setVelocityX(160);
					mario.anims.play('rightmario', true);
				}
				else {
					mario.setVelocityX(0);
					if (direction=='gauche'){
						mario.anims.play('turnleftmario');
					}
					else {
						mario.anims.play('turnrightmario');
					}
				}
				//orienter la bombe
				if (!plus_bombe){
