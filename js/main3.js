...
class SceneSousSol extends Phaser.Scene{
	...
	create(){
		...
		this.physics.add.collider(mario, tortue, this.explosion); //si collision entre mario et tortue appelle collision
		//création des touches
		cursors = this.input.keyboard.createCursorKeys();
		button_p = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.P);
		//création des pieces
...
const config = {
    ...
    scene: [ SceneDepart, SceneGlace, SceneSousSol, ScenePause ]
};
