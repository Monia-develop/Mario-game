	...
	update(){
		...
		if (mario_mort) {
			...
			if (button_p.isDown){ //si on appuie sur a touche P
				...
				if (this.physics.overlap(mario,porte)) {
					this.scene.pause();
...
class ScenePause extends Phaser.Scene{
    ...
    create (){
		...
		//this.input.once('pointerdown', function (){/*text.setText('fin pause');*/image_game_paused.visible=false;this.scene.resume('SceneA');}, this);
    }
...
const config = {
    ...
    scene: [ SceneDepart, ScenePause, SceneGlace ]
};
